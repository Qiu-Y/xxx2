...
class ClassnameAction extends Action {	
    ...
    public function classname(){ 
		...
		if($_GET['clas'])
		{
			...
			$student_list=$student->where("clas='".$_GET['clas']."'")->field(array('id','clas','name','phone','num'))->select();	
		}
		else
		{
			$count=$student->where("clas='通信1101'")->count();
			$student_list=$student->where("clas='通信1101'")->field(array('id','clas','name','phone','num'))->select();	
		}
