<?php
/**
     * @类		IndexAction
     * @功能	后台首页控制器
*/
class IndexAction extends Action {	
	/**
     * @函数	index
     * @功能	显示后台管理主页面（包含登录判断）
     */
    public function index(){ 	
        header("Content-Type:text/html; charset=utf-8");
          
		if(session('?login_username')){
			$this->assign('username',session('login_username'));
			$this->display();
			
		}		
		else
		{
			redirect(U('/Login/index/'));
		}	
    }
  
	
	
	//友情链接	
	function link(){
		header("Content-Type:text/html; charset=utf-8");

		if(session('?login_username')){
			$this->assign('username',session('login_username'));

		
			//实例化文章模型
			$news=M('link');	
			$count=$news->count();
		
			//分页显示文章列表，每页8篇文章
			import('ORG.Util.Page');
			$page=new Page($count,10);//后台管理页面默认一页显示8条文章记录
	
            $page->setConfig('prev', "&laquo; Previous");//上一页
            $page->setConfig('next', 'Next &raquo;');//下一页
            $page->setConfig('first', '&laquo; First');//第一页
            $page->setConfig('last', 'Last &raquo;');//最后一页	
			$page->setConfig('theme',' %first% %upPage%  %linkPage%  %downPage% %end%');
            //设置分页回调方法
			$show=$page->show();
	
			$news_list=$news->field(array('id','addr','info'))->order('id desc')->limit($page->firstRow.','.$page->listRows)->select();
			
			//对原始信息过滤
			$this->filter(&$news_list);
			
			$this->assign('news_count',$count);
			$this->assign('title','后台管理系统');
			$this->assign('news_list',$news_list);
			$this->assign('page_method',$show);
						
			$this->display();
			
		}
		else
		{
			$this->error('您好，请先登录！！！',U('/Login/index/'));
		}	
	}
	
	
	//添加链接
	function addlink(){
		$addr=$_POST["addr"];
		$info=$_POST["info"];
		$Link=D('Link');
		if($Link->create()){
			$Link->addr=$addr;
			$Link->info=$info;
			if($Link->add()){
				$this->success('通知添加成功，返回上级页面',U('Index/link'));
			}else{
				$this->error('文章添加失败，返回上级页面');
			}
			
				
		}
		else{
			$this->error($Link->getError());
		}
	}
	
	
	
	
    
    
	
	
	//目录的展示
	function catalog(){
		
		
		header("Content-Type:text/html; charset=utf-8");
		if(session('?login_username')){
			$this->assign('username',session('login_username'));
			//实例化文章模型'
			$news=M('catalog');				
			$count=$news->count();
			$news_list=$news->field(array('id','f_cata','isf','f_id','p_id'))->select();	
			$this->assign('news_count',$count);
			$this->assign('title','后台管理系统');
			$this->assign('news_list',$news_list);
			$this->assign('page_method',$show);
						
			$this->display();
			
		}
		
		else
		{
			$this->error('您好，请先登录！！！',U('/Login/index/'));
		}		
	}
	
	//添加目录
	function addcatalog(){
		$name=$_POST['catalog'];
		if($name=="")
		{
			$this->error("内容不能为空");
		}
		else if(strlen($name)>20)
		{
			$this->error("主目录不过长");	
		}
		else
		{
			$catalog=M('catalog');
			if($catalog->where('f_cata='.$name)->find())
			{
				
				$this->error("该字段已经存在！");
			}
			else
			{
				$catalog=D('Catalog');
				if($catalog->create())
				{
					$catalog->f_cata=$name;
					$catalog->f_id=0;
					if($catalog->add())
					{
						$this->success("目录添加成功！");	
					}
					else
					{
						$this->error("数据添加失败");	
					}
					
				}
				else
				{
					$this->error("数据库读取失败");	
				}
			}
				
		}	
	}
	
	
	//添加二级目录
	function add_second(){	
		$news=M('catalog');			
		if($_GET['id'])	
		{
			$id=$_GET['id'];
			$news_list=$news->where('id='.$id)->field(array('id','f_cata','isf','f_id','p_id'))->select();	
		}
		else{
			$news_list=$news->where("f_id='0'")->field(array('id','f_cata','isf','f_id','p_id'))->select();	
		}
		$this->assign('news_list',$news_list);
		$this->assign('title','后台管理系统');
		$this->display();
		
	}
	
	function add_s(){
		$fir=$_POST['sel'];
		$nn=$_POST['second'];
		$isf=$_POST['checkbox'];
		
		if($nn=="")
		{
			$this->error("二级目录内容不能为空");
		}
		else if(strlen($nn)>20)
		{
			$this->error("目录不能过长");	
		}
		else
		{
			$catalog=M('catalog');
			if($catalog->where("f_cata='".$nn."'")->field(array('id','f_cata'))->find())
			{
				$this->error("该字段已经存在！");
			}
			else
			{
				$id=$catalog->where("f_cata='".$fir."'")->field(array('id','f_cata'))->find();
				if($id)
				{
					$catalog=D('Catalog');	
					if($catalog->create())
					{
						if($isf=="isf")
						{
							$isf_name=1;	
						}
						else{
							$isf_name=0;	
						}
						$catalog->f_cata=$nn;
						$catalog->f_id=$id['id'];
						$catalog->isf=$isf_name;
						
						
						if($catalog->add())
						{
							$this->success("二级目录添加成功！",U('/Index/catalog'));	
						}
						else
						{
							$this->error("数据添加失败");	
						}
						
					}
					else
					{
						$this->error("数据库读取失败");	
					}
				}
			}
			
				
		}	
		//redirect(U('/Index/catalog'));
		
	}
	
	//目录删除
	function delete_catalog(){
    	$catalog=M('catalog');
		$id=$_GET['id'];
		$f_id=$catalog->where("id='$id'")->field(array('id','f_id'))->find();
		if($f_id['f_id']=='0')
		{
			$catalog->where("f_id='".$f_id['id']."'")->delete();
			$catalog->delete($_GET['id']);
			$this->success('目录删除成功');
			
		}
		else
		{
			if($catalog->delete($_GET['id']))
			{
				$this->success('目录删除成功');
			}
			else
			{
				$this->error($article->getLastSql());
			}
		}
    }
	
	
	
	
	
    
  /**
     * @函数	article
     * @功能	编辑新的文章
     */
   
    /**
     * @函数	delete
     * @功能	删除文章
     */
    function delete(){
    	
    	//跳转至Article控制器来实现
    	if($_GET['id']){
    		redirect(U('/Article/delete/id/'.$_GET['id']),0, '删除文章');
    	}
    	else{
    		$this->error('参数错误！');
    	}
    }
	
	//文章查找
	
	function find(){
		$title=$_POST['mulu'];
		$select=$_POST['select'];			
	    //实例化文章模型	
			
			$news=M('cata');	
			$news_list=$news->where("title='$title=' and Catalog='$select'")->order('id desc')->limit($page->firstRow.','.$page->listRows)->select();
			$count=$news->count();
			dump($news_list);
			
			
			$cata=M('catalog');
			$mulu=$cata->where("isf=0")->select();
			$this->assign('mulu',$mulu);
			
			
			//分页显示文章列表，每页8篇文章
			import('ORG.Util.Page');
			$page=new Page($count,8);//后台管理页面默认一页显示8条文章记录
	
            $page->setConfig('prev', "&laquo; Previous");//上一页
            $page->setConfig('next', 'Next &raquo;');//下一页
            $page->setConfig('first', '&laquo; First');//第一页
            $page->setConfig('last', 'Last &raquo;');//最后一页	
			$page->setConfig('theme',' %first% %upPage%  %linkPage%  %downPage% %end%');
            //设置分页回调方法
			$show=$page->show();
			
			//对原始信息过滤
			$this->filter(&$news_list);
			
			$this->assign('news_count',$count);
			$this->assign('title','后台管理系统');
			$this->assign('news_list',$news_list);
			$this->assign('page_method',$show);
						
			$this->display();
	}
	//编辑目录
	function edit_catalog(){
		$id=$_GET['id'];
		$catalog=M('catalog');
		$catalog_list=$catalog->field(array('id','f_cata','s_cata','f_id'))->where('id='.$id)->find();
		
	
		$this->assign('catalog_list',$catalog_list);
		$this->display();
		
			
	}
	
	function edit_log(){
		$catalog=M('catalog');
		$f_cata=$_POST['f_cata'];
		$s_cata=$_POST['s_cata'];
		$id=$_POST['id'];
		if($f_cata==""&&$s_cata=="")
		{
			$this->error('不能为空！');
		}
		else
		{
			$data = array('f_cata'=>$f_cata,'s_cata'=>$s_cata);

			if($catalog->where('id='.$id)->setField($data))
			{
				$this->success('修改成功',U('Index/catalog'));	
			}
			else
			{
				$this->error($catalog->getLastSql());
			}
				
		}
	}
	
	
    
    /**
     * @函数	edit
     * @功能	编辑文章
     */
    function edit(){
    	if($_GET['id']){
    		redirect(U('/Article/edit/id/'.$_GET['id']),0, '编辑文章');
    	}
    	else{
    		$this->error('参数错误！');
    	}
    } 
	
	
	
	
	
	
	/**
     其他链接
     */
    function quit(){
    	session(null);//清空所有session信息
		redirect(U('/Login/index'),0, '重新登录');
    }
	
	function tzlis(){
		redirect(U('/Inform/tzlis/'));
		
	}
	
	function add_tz(){
		redirect(U('/Inform/add_tz/'));
	}
	
	function classname()
	{
		redirect(U('/Classname/classname'),0, '学生管理');
	}
	
	function add_class()
	{
		redirect(U('/Classname/add_class'),0, '添加班级');	
	}
	
	function class_lis(){
		redirect(U('/Classname/class_lis'),0, '班级管理');		
	}
	
	//回到首页
	function in(){
		redirect(U('/Index/index'),0, '回到首页');
	}
	
	function lis(){
		redirect(U('/Article/lis'),0, '新闻修改');
	}
	function article(){
    	//跳转
    	redirect(U('/Article/index'),0, '新闻添加');
    } 
	function teachername(){
		redirect(U('/Classname/teachername'),0, '添加用户');
	}
	
	function film_add(){
		redirect(U('/Film/film_add'),0, '视频添加');
	}
	
	function film_mag(){
		redirect(U('/Film/film_mag'),0, '视频添加');
	}
	
    
/*--------------------------------------------------内部方法-------------------------------------------------------------------*/    
     /**
     * @函数	filter
     * @功能	对数据库中的信息进行裁剪和过滤
     */ 
    private function filter($list){
    		
    	foreach($list as $key=>$value){			
   			//设置显示的创建时间
			$list[$key]['createtime']=date("Y-m-d H:i:s",$value['createtime']);
				
			//设置显示的最后修改时间
			if(!$value['lastmodifytime']){
				$list[$key]['lastmodifytime']="无";
			}else{
				$list[$key]['lastmodifytime']=date("Y-m-d H:i:s",$value['lastmodifytime']);
			}		
			
			//文章标题过长时裁剪
			if(strlen($list[$key]['subject'])>80){
					$list[$key]['subject']=$this->cutString($list[$key]['subject'],0,20).'...';				
			}
		}
    }
    
     /**
     * @函数	cutString
     * @功能	字符串裁剪(仅适用于UTF-8)
     */	
	private function cutString($str, $from, $len)
	{
   		return preg_replace('#^(?:[\x00-\x7F]|[\xC0-\xFF][\x80-\xBF]+){0,'.$from.'}'.
                       '((?:[\x00-\x7F]|[\xC0-\xFF][\x80-\xBF]+){0,'.$len.'}).*#s',
                       '$1',$str);
	}
}

